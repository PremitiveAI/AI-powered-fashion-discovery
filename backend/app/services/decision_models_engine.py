...
from app.core.models_prompt import MODELS_PROMPT
...
async def analyze_person_mode(resized_image, detections):
    ...
    for idx, det in enumerate(persons[:1]):
        ...
        attrs = await describe(person_crop, MODELS_PROMPT)
...
async def analyze_object_mode(resized_image):
    attrs = await describe(resized_image, MODELS_PROMPT)
...
def choose_best_bbox(
    ...
    return {
        **{k: obj.get(k) for k in ("category","type","subtype","color","brand","gender","pattern")},
        "relation": relation,
        "confidence": conf
    }
