...
export default function FashionDashboard() {
  ...
  useEffect(() => {
    if (!gender || !selectedCategoryIds.length) return;
    ...
    const fetchRecommendations = async () => {
      try {
        ...
        const res = await fetch("/api/phase2/model-list", {
          method: "POST",
  ...
  }, [gender, selectedCategoryIds]);

  const handleTryOn = async (modelId: number) => {
    try {
      setShowGenerating(true);

      const res = await fetch("/api/phase2/user-try-on", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ user_id: profileId, model_id: modelId }),
    });
    ...
    const data = await res.json();
      ...
      if (data?.image_url) {
      setTryOnResults((prev) => ({ ...prev, [modelId]: data.image_url }));
    }
    } catch (error) {
      console.error("Failed to fetch categories:", error);
    } finally {
      setTryOnLoading((prev) => ({ ...prev, [modelId]: false }));
      setShowGenerating(false);
    }
  };

  const handleLookDetail = (modelId: number) => {
    setSelectedModelId(modelId);
    router.push(`/phase2/look-detail?modelId=${modelId}&profileId=${profileId}`);
  };
                    ...
                    <section className="bg-[#121212] rounded-[3rem] p-10 space-y-12">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-1 px-2">
                        {group.models.map((model) => {
                          const displayImage = tryOnResults[model.id] || model.user_url || model.model_url;
                          return (
                                ...
                                {model.items.slice(0, 2).map((item) => (
                                  <div key={item.id} className="flex items-center gap-3">
                                    <div className="w-18 h-18 rounded-full bg-gray-500 overflow-hidden">
                                      <img
                                        src={item.product_list?.[0]?.images?.[0]?.path ?? model.model_url}
                                        alt={item.type}
                      ...
                      {group.models.map((model) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            {/* Right Glass Panel */}
                            <div
                              className="absolute right-0 top-0 h-full w-[40%] flex flex-col justify-between px-3 py-5"
                              style={{
