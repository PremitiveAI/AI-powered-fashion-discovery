...
export default function LookDetailPage() {
  ...
  const displayImage = tryOn || lookData?.user_url  || lookData?.model_url;
  const isModelImage = !tryOn && !lookData?.user_url && !!lookData?.model_url;
  ...
  const fetchTryOn = async () => {
   ...
   const fetchLookDetails = async () => {
      try {
        ...
        const res = await fetch("/api/phase2/look-detail", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(payload),
        });
    ...
    <DashboardLayout>
      <div className="min-h-screen bg-[#0d0f14] text-white px-10 py-12">
        ...
        <div className="grid grid-cols-[1fr_1.3fr] gap-16">
          {!loading && (
            <div className="relative ml-20 mr-20 aspect-[4/5] max-h-[700px] rounded-3xl p-[2px] bg-gradient-to-br from-purple-500 via-fuchsia-500 to-indigo-500">
          ...
          {lookData && (
            <div>
              ...
              <div>
                ...
                {lookData.items.slice(0, 3).map((item: any) => (
                  <div
                    key={item.id}
                  ...
                  >
                    <div className="flex items-center gap-4">
                      ...
                      <div className="w-18 h-18 rounded-full bg-gray-500 overflow-hidden">                        <img
                          src={item.product_list?.[0]?.images?.[0]?.path ?? lookData.model_url}
                          alt={item.type}
            ...
            <div className="grid grid-cols-4 gap-10">
              {lookData.similar_models.map((model: any) => (
                ...
                >
                  ...
                  <img
                    src={model.user_url || model.model_url}
